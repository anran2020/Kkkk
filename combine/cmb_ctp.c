...
// 12	DATA2-1	2byte	x	电流量程，上传通道的电流量程，单位A										
	...
	pBox->vMax=prxfrm->u16Data2*1000;
	pBox->iMax=prxfrm->u16Data1*1000;
...
// 8	DATA4-1	4byte	x						电流，通道的电流采样数据，单位0.01mA				
	...
	pmdl=pBox->mdl+indx;
	pmdl->sta=((prxfrm->sfRxIDbit.DATA10>>4)&0x0f);
	pmdl->DDC=prxfrm->sfRxIDbit.DATA9;
	pmdl->current=prxfrm->u32Data1;
	pmdl->voltage=prxfrm->u32Data2;
	//上报采样成功
	return mltrue;
}
