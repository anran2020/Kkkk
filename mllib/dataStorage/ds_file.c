...
{
	if(!dsfile->bOpened||line>=dsfile->lineMax)
	{
		return mlfalse;
	}

	mlu8 i;
	mlu32 startPageAddr=dsfile->pagesPerLine;
	startPageAddr=dsfile->basePage+DS_FILE_BACKUP_PAGE_NUM+line*startPageAddr;//计算 页地址

	mlu32 pageAddr=startPageAddr;
	//一行多页的写入
	for ( i = 0; i < dsfile->pagesPerLine; i++)
	{
		...
		pdat+=DS_FLASH_PAGE_SIZE;//偏移页长度，继续写入
	}

	//文件行维护，文件写入行，发生变化
	dsfile->header.offsetLine=line+1;		//偏移到文件尾，即下一个写入行号
	...
	//打印日志
	log_print(LOG_LVL_INFO,"file[%d] w line %d page  %d ",dsfile->hdl,line,startPageAddr);
