...
//
//-----------------------------------------------------------------------------
mlu8* Pb_plc_txbuf_malloc(mlu8 plcx,mlu16 len)
{
    ...
    if (prsBed.usock[plcx]->sta != e_sck_sta_working)
    {
    	prsBed.usock[plcx]->txDataLen = 0;
        return nullptr;
    }

    if((prsBed.usock[plcx]->txDataLen+len)<=prsBed.usock[plcx]->txSize)
    {
        u8ptr = prsBed.usock[plcx]->pTxBuf + prsBed.usock[plcx]->txDataLen;
        prsBed.usock[plcx]->txDataLen += len;
    }
...
{
    usocket_reset(prsBed.usock[plcx]);
}
...
Taskstate Pb_task(void *args)
{
    ...
    for(i=0;i<PB_CTRL_DEV_MAX;++i)
    {
        if(prsBed.usock[i]!=nullptr)
        {
            ...
            {
                prsBed.usock[i]->rxDataLen = prsBed.datagramProcesses[i](i, prsBed.usock[i]->pRxBuf,prsBed.usock[i]->rxDataLen);
            }
...
//
//-----------------------------------------------------------------------------
void Pb_plc_socket_init(mlu8 plcx,mlu8 *remoteip, mlu16 remoteport,mlu16 (*datagramProcesses)(mlu8 plcIdx, mlu8 *msg,mlu16 len))
{
    mlu8 i;
    if(plcx>=PB_CTRL_DEV_MAX)
        return;
