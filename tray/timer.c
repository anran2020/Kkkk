...
void timerStart(Timer *timer, u8eTimerType type, u32 mSec, b8 reset)
{
    ...
    exprTime->mSec += mSec % 1000;
    exprTime->sec += mSec/1000;
...
void timerInit()
{
    ...
    mgr->exprProc[TidUpDisc] = trayUpDiscExpr;
    mgr->exprProc[TidNdbdBrkWaitNp] = trayNdbdBreakChk;
    mgr->exprProc[TidCanRxCellSwAck] = boxExprCellSwAck;
    mgr->exprProc[TidTrayMntnExpr] = trayMntnExpr;
#endif
    return;
}
