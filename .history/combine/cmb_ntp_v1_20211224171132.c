...
unsigned char cblntp_receive_workstep(void *args)
{
	...
	for(i=0;i<worksteps;i++)
	{
		...
		if(pStepBuf[i*STEP_BYTES+1]==pChnl->rcvedStepCnt)
		{
			...
			pChnl->steps[n].lPara1.nextStep=pStepBuf[i*STEP_BYTES+12];		

			// 12 -19 lpara2 u64
			u8_to_u32( pStepBuf[i*STEP_BYTES+17], pStepBuf[i*STEP_BYTES+18], 
			pStepBuf[i*STEP_BYTES+19],pStepBuf[i*STEP_BYTES+20], temp1);
			pChnl->steps[n].lPara2.value=temp1;
			pChnl->steps[n].lPara2.value=(pChnl->steps[n].lPara2.value<<32);			

			u8_to_u32(pStepBuf[i*STEP_BYTES+13], pStepBuf[i*STEP_BYTES+14], 
				pStepBuf[i*STEP_BYTES+15],pStepBuf[i*STEP_BYTES+16],  temp1);				
			pChnl->steps[n].lPara2.value+=temp1;		
			
			pChnl->steps[n].lPara2.handleWay=pStepBuf[i*STEP_BYTES+21];
			pChnl->steps[n].lPara2.nextStep=pStepBuf[i*STEP_BYTES+22];

			// 22 -25 lpara3
			u8_to_u32( pStepBuf[i*STEP_BYTES+23], pStepBuf[i*STEP_BYTES+24], 
			pStepBuf[i*STEP_BYTES+25],pStepBuf[i*STEP_BYTES+26], temp1);		
			pChnl->steps[n].lPara3.value=temp1;
			pChnl->steps[n].lPara3.handleWay=pStepBuf[i*STEP_BYTES+27];
			pChnl->steps[n].lPara3.nextStep=pStepBuf[i*STEP_BYTES+28];

			//28-31 lpara4
			u8_to_u32( pStepBuf[i*STEP_BYTES+29], pStepBuf[i*STEP_BYTES+30], 
				pStepBuf[i*STEP_BYTES+31], pStepBuf[i*STEP_BYTES+32],temp1);			
			pChnl->steps[n].lPara4.value=temp1/10;
			pChnl->steps[n].lPara4.handleWay=pStepBuf[i*STEP_BYTES+33];
			pChnl->steps[n].lPara4.nextStep=pStepBuf[i*STEP_BYTES+34];	
