...
//
//-----------------------------------------------------------------------------
MLBool cblntp_sample(void *args)
{
...
unsigned char cblntp_receive_workstep(void *args)
{
	...
	if(pbox->online==_reset||prxbuf[4]>=pbox->)//设备是否在线
	{
		return mltrue;
	}
	...
	pStepBuf=prxbuf+5;
	pChnl->workStepRealCnt=pStepBuf[0];
	...
	for(i=0;i<worksteps;i++)
	{
		...
		{
			pChnl->workStepRcvCnt=0;
		}
		
		if(pStepBuf[i*IGBT_One_Step_Len+1]==pChnl->workStepRcvCnt)
		{
			n=pChnl->workStepRcvCnt;
				
			pChnl->workStep[n].type=pStepBuf[i*IGBT_One_Step_Len+2];
			sys.steptype =	pChnl->workStep[n].type;
			//2345 mpara1
			...
			if((pChnl->workStep[n].type==WST_CCCV)||(pChnl->workStep[n].type==WST_CCD)
				||(pChnl->workStep[n].type==WST_DCIR)||(pChnl->workStep[n].type==WST_CVPC)
				||(pChnl->workStep[n].type==WST_CCPC)||(pChnl->workStep[n].type==WST_CVD)||(pChnl->workStep[n].type==WST_CCCV2))
			{
				pChnl->workStep[n].paraTemplt.mPara1=temp1/10;
			}
			else
			{
				pChnl->workStep[n].paraTemplt.mPara1=temp1;
			}
			...
			if(pChnl->workStep[n].type==WST_JUMP)
			{
				pChnl->workStep[n].paraTemplt.lPara1.value=temp1;
			}
			else
			{
				pChnl->workStep[n].paraTemplt.lPara1.value=temp1/10;
			}
			pChnl->workStep[n].paraTemplt.lPara1.handleWay=pStepBuf[i*IGBT_One_Step_Len+11];	
			pChnl->workStep[n].paraTemplt.lPara1.nextStep=pStepBuf[i*IGBT_One_Step_Len+12];		
			...
			pChnl->workStep[n].paraTemplt.lPara2.value=temp1;
			pChnl->workStep[n].paraTemplt.lPara2.value=(pChnl->workStep[n].paraTemplt.lPara2.value<<32);			
			...
			pChnl->workStep[n].paraTemplt.lPara2.handleWay=pStepBuf[i*IGBT_One_Step_Len+21];
			pChnl->workStep[n].paraTemplt.lPara2.nextStep=pStepBuf[i*IGBT_One_Step_Len+22];
			...
			pChnl->workStep[n].paraTemplt.lPara3.value=temp1;
			pChnl->workStep[n].paraTemplt.lPara3.handleWay=pStepBuf[i*IGBT_One_Step_Len+27];
			pChnl->workStep[n].paraTemplt.lPara3.nextStep=pStepBuf[i*IGBT_One_Step_Len+28];
			...
			pChnl->workStep[n].paraTemplt.lPara4.value=temp1/10;
			pChnl->workStep[n].paraTemplt.lPara4.handleWay=pStepBuf[i*IGBT_One_Step_Len+33];
			pChnl->workStep[n].paraTemplt.lPara4.nextStep=pStepBuf[i*IGBT_One_Step_Len+34];	
			...
			pChnl->workStep[n].sampleInterval=temp1;
...
// 				if(pChnl->workStep[n].type==WST_DCIR)
					pChnl->workStep[n].paraTemplt.mPara2=temp1;
...
// 				if(pChnl->workStep[n].type==WST_DCIR)
					pChnl->workStep[n].paraTemplt.mPara3=temp1;
			...
			pChnl->workStep[n].paraTemplt.mPara4=temp1;
			...
			pChnl->workStep[n].paraTemplt.mPara5=temp1;		
			
			pChnl->workStepRcvCnt++;
  ...
  // chnlx=prxbuf[4];
	...
	{  
		u8_to_u32(prxbuf[i*11+11+0], prxbuf[i*11+11+1], prxbuf[i*11+11+2],
			prxbuf[i*11+11+3], pbox->chnl[prxbuf[4]].wcsiu[wcsiuIndex].subWorkStep[n].para1);
		
		u8_to_u32(prxbuf[i*11+11+4], prxbuf[i*11+11+5], prxbuf[i*11+11+6],
			prxbuf[i*11+11+7], pbox->chnl[prxbuf[4]].wcsiu[wcsiuIndex].subWorkStep[n].para2);	
     
		u8_to_u16(prxbuf[i*11+11+8], prxbuf[i*11+11+9], 
				pbox->chnl[prxbuf[4]].wcsiu[wcsiuIndex].subWorkStep[n].runTime);

		pbox->chnl[prxbuf[4]].wcsiu[wcsiuIndex].subWorkStep[n].type=prxbuf[i*11+11+10];

		//处理参数
		if((pbox->chnl[prxbuf[4]].wcsiu[wcsiuIndex].subWorkStep[n].type==WCS_TYPE_CCC)||
			(pbox->chnl[prxbuf[4]].wcsiu[wcsiuIndex].subWorkStep[n].type==WCS_TYPE_CCD))
		{
			pbox->chnl[prxbuf[4]].wcsiu[wcsiuIndex].subWorkStep[n].para1/=10;
			pbox->chnl[prxbuf[4]].wcsiu[wcsiuIndex].subWorkStep[n].para2/=10;
		}
