...
unsigned char cblntp_connect(void*args)
{
	...
	//命令字
	ptxbuf[7]=CMD_Connect;

	//设备通道数
	ptxbuf[8]=emu.igbt.onlineChnlNum;	
...
unsigned char cblntp_sample(void *args)
{
	...
	u8 chnlx,*ptr,*ptxbuf,j,*pNetRxBuf;
...
//	len=4+2+6+10*3+1;
	...
	//地址域
	ptxbuf[6]=pNetRxBuf[2];
	
	//命令字
	ptxbuf[7]=CMD_GetStack;	

	ptxbuf[8]=pNetRxBuf[4];
	
	ptxbuf[9]=OK_FLAG;
	
	ptxbuf[10]=emu.igbt.channel[pNetRxBuf[4]].DDC;

	//堆栈深度
	ptxbuf[11]=emu.igbt.channel[pNetRxBuf[4]].stackDepth;
	
	//堆栈信息
	for(i=0;i<30;i++)
	{
		ptxbuf[12+i*3+0]=emu.igbt.channel[pNetRxBuf[4]].stack[i].pushStepNumber;
		ptxbuf[12+i*3+1]=emu.igbt.channel[pNetRxBuf[4]].stack[i].popValue;
		ptxbuf[12+i*3+2]=(emu.igbt.channel[pNetRxBuf[4]].stack[i].popValue>>8);	
	}
...
//	len=4+2+3+4+6+4+4+1+1;
	...
	ptxbuf[9]=pNetRxBuf[6];
	ptxbuf[10]=pNetRxBuf[7];
	ptxbuf[11]=pNetRxBuf[8];
	ptxbuf[12]=pNetRxBuf[9];
...
//	7	校验码	1	CHK	从箱号地址到数据体的校验和
	...
	ptxbuf[8]=pNetRxBuf[4];
	 ptxbuf[9]=OK_FLAG;
	 ptxbuf[10]=emu.igbt.channel[pNetRxBuf[4]].stackDepth;
	 
	for(i=0;i<Stack_Depth_Max;i++)
	{
		 ptxbuf[11+i*3]=emu.igbt.channel[pNetRxBuf[4]].stack[i].pushStepNumber;
		 ptxbuf[11+i*3+1]=emu.igbt.channel[pNetRxBuf[4]].stack[i].popValue;
		 ptxbuf[11+i*3+2]=(emu.igbt.channel[pNetRxBuf[4]].stack[i].popValue>>8);
	}
...
unsigned char cblntp_setup_parallel(void *args)
 {
	u8 *pNetRxBuf,i,subChnlNum,*ptxbuf;
	u8 error,OorE;
	Module_TypeDef *pMdl;
	pNetRxBuf=(u8*)args;
