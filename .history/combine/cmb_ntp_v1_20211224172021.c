...
//
//-----------------------------------------------------------------------------
void cmb_ntp_cmd_reply(mlu8 cmd,mlu8 addr,mlu8 chnl,mlu8 oeFlag,mlu8 errorMsg)
{
	u8 *pNetTxBuf,i;
	
	//获取发送缓冲区
	pNetTxBuf=w5100_tx_buf_get(11);
	if(pNetTxBuf==_NULL)
		return ;
...
// 				if(pChnl->steps[n].type==STEP_DCIR)
	...
	//接收返回
	cmb_ntp_cmd_reply(CMD_WorkStepInfo, prxbuf[2],prxbuf[4] ,retFlag,0);
	return mltrue;
}
#if 0
/****************************************************************************
