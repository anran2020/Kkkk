...
unsigned char cblntp_receive_workstep(void *args)
{
	...
	for(i=0;i<worksteps;i++)
	{
		...
		if(pStepBuf[i*STEP_BYTES+1]==pChnl->rcvedStepCnt)
		{
			...
			sys.steptype =	pChnl->steps[n].type;
			//2345 mpara1
			u8_to_u32(pStepBuf[i*STEP_BYTES+3], pStepBuf[i*STEP_BYTES+4], 
						pStepBuf[i*STEP_BYTES+5], pStepBuf[i*STEP_BYTES+6], temp1);

			if((pChnl->steps[n].type==WST_CCCV)||(pChnl->steps[n].type==WST_CCD)
				||(pChnl->steps[n].type==WST_DCIR)||(pChnl->steps[n].type==WST_CVPC)
				||(pChnl->steps[n].type==WST_CCPC)||(pChnl->steps[n].type==WST_CVD)||(pChnl->steps[n].type==WST_CCCV2))
			{
				pChnl->steps[n].paraTemplt.mPara1=temp1/10;
			}
			else
			{
				pChnl->steps[n].paraTemplt.mPara1=temp1;
			}
			
			// 6789 lpara1
			u8_to_u32( pStepBuf[i*STEP_BYTES+7], pStepBuf[i*STEP_BYTES+8],
			pStepBuf[i*STEP_BYTES+9], pStepBuf[i*STEP_BYTES+10],temp1);
			if(pChnl->steps[n].type==WST_JUMP)
			{
				pChnl->steps[n].paraTemplt.lPara1.value=temp1;
			}
			else
			{
				pChnl->steps[n].paraTemplt.lPara1.value=temp1/10;
			}
			pChnl->steps[n].paraTemplt.lPara1.handleWay=pStepBuf[i*STEP_BYTES+11];	
			pChnl->steps[n].paraTemplt.lPara1.nextStep=pStepBuf[i*STEP_BYTES+12];		

			// 12 -19 lpara2 u64
			u8_to_u32( pStepBuf[i*STEP_BYTES+17], pStepBuf[i*STEP_BYTES+18], 
			pStepBuf[i*STEP_BYTES+19],pStepBuf[i*STEP_BYTES+20], temp1);
			pChnl->steps[n].paraTemplt.lPara2.value=temp1;
			pChnl->steps[n].paraTemplt.lPara2.value=(pChnl->steps[n].paraTemplt.lPara2.value<<32);			

			u8_to_u32(pStepBuf[i*STEP_BYTES+13], pStepBuf[i*STEP_BYTES+14], 
				pStepBuf[i*STEP_BYTES+15],pStepBuf[i*STEP_BYTES+16],  temp1);				
			pChnl->steps[n].paraTemplt.lPara2.value+=temp1;		
			
			pChnl->steps[n].paraTemplt.lPara2.handleWay=pStepBuf[i*STEP_BYTES+21];
			pChnl->steps[n].paraTemplt.lPara2.nextStep=pStepBuf[i*STEP_BYTES+22];

			// 22 -25 lpara3
			u8_to_u32( pStepBuf[i*STEP_BYTES+23], pStepBuf[i*STEP_BYTES+24], 
			pStepBuf[i*STEP_BYTES+25],pStepBuf[i*STEP_BYTES+26], temp1);		
			pChnl->steps[n].paraTemplt.lPara3.value=temp1;
			pChnl->steps[n].paraTemplt.lPara3.handleWay=pStepBuf[i*STEP_BYTES+27];
			pChnl->steps[n].paraTemplt.lPara3.nextStep=pStepBuf[i*STEP_BYTES+28];

			//28-31 lpara4
			u8_to_u32( pStepBuf[i*STEP_BYTES+29], pStepBuf[i*STEP_BYTES+30], 
				pStepBuf[i*STEP_BYTES+31], pStepBuf[i*STEP_BYTES+32],temp1);			
			pChnl->steps[n].paraTemplt.lPara4.value=temp1/10;
			pChnl->steps[n].paraTemplt.lPara4.handleWay=pStepBuf[i*STEP_BYTES+33];
			pChnl->steps[n].paraTemplt.lPara4.nextStep=pStepBuf[i*STEP_BYTES+34];	
