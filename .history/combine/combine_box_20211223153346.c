...
{
	CmbPwrBoxptr pPwrbox;

	pPwrbox=mlos_malloc(e_mem_sram,sizeof(CombinePowerBox));//箱分配内存
	pPwrbox->mdl=mlos_malloc(e_mem_sram,sizeof(CombinePowerModule)*chnlNum);//模块分配内存
	pPwrbox->chnl=mlos_malloc(e_mem_sram,sizeof(CombinePowerChannel)*chnlNum);//通道分配内存
	
	for (mlu8 i = 0; i < chnlNum; i++)
	{
		mdl_init(pPwrbox->mdl+i);//初始化模块
		chnl_init(pPwrbox->chnl+i);//初始化通道
		pPwrbox->chnl[i]->mdlList->head=pPwrbox->mdl+i;//一个通道初始对应一个模块
	}
