...
MLBool cblntp_sample(void *args)
{
	...
	sdIndex=sdIndex-(chnlx*Igbt_ChnlOneDataRec_Len*CMB_CH_DAT_RCRD_CNT);
...
// 6	当前写入地址	4		
	...
	temp=emu.igbt.channel[chnlx].dataSaveEndAddr+
			((u32)CMB_CH_DAT_RCRD_CNT*Igbt_ChnlOneDataRec_Len*chnlx);
...
//	len=4+2+3+4+6+4+4+1+1;
	...
	// falsh 最大地址
	len =CMB_CH_DAT_RCRD_CNT*Igbt_ChnlOneDataRec_Len;
	...
	temp=emu.igbt.channel[prxbuf[4]].dataSaveEndAddr+
				(prxbuf[4]*CMB_CH_DAT_RCRD_CNT*Igbt_ChnlOneDataRec_Len);
...
unsigned char cblntp_continue_test(void *args)
{
	...
	ptxbuf[9]=OK_FLAG;
		...
		temp=emu.igbt.channel[prxbuf[4]].dataSaveEndAddr+
			((u32)CMB_CH_DAT_RCRD_CNT*Igbt_ChnlOneDataRec_Len*prxbuf[4]);
