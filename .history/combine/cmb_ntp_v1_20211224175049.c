...
//
//-----------------------------------------------------------------------------
MLBool cmb_receive_step(void *args)
{
...
//
//-----------------------------------------------------------------------------
MLBool cmb_upload_stack(void *args)
{
...
//	len=4+2+6+10*3+1;
	...
	ptxbuf[4]=((len-6)&0xff);//数据包数据长度
	ptxbuf[5]=(((len-6)>>8)&0xff);
	ptxbuf[6]=prxbuf[2];//地址域
	ptxbuf[7]=CMD_GetStack;	//命令字
	ptxbuf[8]=prxbuf[4];
	ptxbuf[9]=OK_FLAG;
	...
	for(i=0;i<30;i++)
	{
		ptxbuf[12+i*3+0]=pbox->chnl[prxbuf[4]].stack[i].pushStepNo;
...
//---------------------------------------------------------------------------
MLBool cmb_start_test(void  *args)
{	
	mlu8 *ptr,i,*ptxbuf,*prxbuf;
	mlu16 len ;
	mlu32 temp;
	...
	//pbox->chnl[prxbuf[4]].stepNum=prxbuf[5];//取启动参数
...
//	len=4+2+3+4+6+4+4+1+1;
	...
	ptxbuf[4]=((len-6)&0xff);//数据包数据长度
	ptxbuf[5]=(((len-6)>>8)&0xff);
	ptxbuf[6]=attri.address;//地址域
	ptxbuf[7]=CMD_StartChnl;//命令字	
	ptxbuf[8]=prxbuf[4];//通道号
	ptxbuf[9]=prxbuf[6];
	ptxbuf[10]=prxbuf[7];
	ptxbuf[11]=prxbuf[8];
	ptxbuf[12]=prxbuf[9];
...
void cmb_ntp_init(void)
{
	...
	upper_consumer_register(CMD_GetStack,cmb_upload_stack);
