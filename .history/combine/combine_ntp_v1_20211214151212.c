...
* 说    明： 
****************************************************************************/
void igbtNtp_req_sub_step(u8 equipAddr,u8 chnlx, u8 mainStepNum )
{
	u8 i,*pNetTxBuf;
	u16 len ;
...
*  
****************************************************************************/
unsigned char igbtNtp_connect(void*args)
{
...
*  
****************************************************************************/
unsigned char igbtNtp_sample(void *args)
{
...
****************************************************************************/
unsigned char igbtNtp_receive_workstep(void *args)
{
...
// 12	重复9,11步骤			
//--------------------------------------------------------------------------------
unsigned char igbtNtp_receive_wcsi(void *args)
{
...
*ntp---- net transmit protocol 
****************************************************************************/
unsigned char igbtNtp_upload_stack(void *args)
{
	u8 i,*pNetTxBuf,*pNetRxBuf;
	u16 len ;
	pNetRxBuf=(u8*)args;
...
// 7	电脑同步时间	4		
//---------------------------------------------------------------------------
unsigned char igbtNtp_start_test(void  *args)
{	
...
// 5	运行通道号	1		
//---------------------------------------------------------------------------
unsigned char igbtNtp_stop_test(void * args)
{	
	u8 *pNetRxBuf;
//	u16 len ;
	pNetRxBuf=(u8*)args;
...
// 7	电脑同步时间	4		
//---------------------------------------------------------------------------
unsigned char igbtNtp_force_jump(void *args)
{	
	u8 *ptr,i,*pNetTxBuf,*pNetRxBuf;
	u16 len ;
	u32 temp;
...
*  
****************************************************************************/
unsigned char igbtNtp_reply_subStep_request(void *args)
{
	u8 *pNetRxBuf;
...
*  
****************************************************************************/
unsigned char igbtNtp_lost_connect(void *args)
{
	u8 i;
...
*ntp---- net transmit protocol 
****************************************************************************/
unsigned char igbtNtp_pause_test(void *args)
{
...
//---------------------------------------------------------------------------
unsigned char igbtNtp_continue_test(void *args)
{
...
*  
****************************************************************************/
unsigned char igbtNtp_setup_parallel(void *args)
 {
...
*  
****************************************************************************/
unsigned char igbtNtp_destroy_parallel(void *args)
{
	u8 *pNetRxBuf,i,*pNetTxBuf;
	u8 error, OorE;
	Module_TypeDef *pMdl;
...
*  
****************************************************************************/
unsigned char igbtNtp_clear_all_parallel(void *args)
{
	//u8 i,j;
	u8 j;
	Module_TypeDef *pMdl;
...
*  
****************************************************************************/
unsigned char igbt_alarmled_cmd(void *args)
{
	u8 * pNetRxBuf;
	pNetRxBuf=(u8 *)args;
	if(*(pNetRxBuf+3)==ALARM_LIGHT_CMD)
	{
		alarm_light_process(&pNetRxBuf[3]);;
	}
	return 1;
}
/****************************************************************************
...
*  
****************************************************************************/
void igbt_ntp_init(void)
{
/*
	//协议处理者
	ntpMU._CMD_connect_handler=igbtNtp_connect;
	ntpMU._CMD_workStepInfo_handler=igbtNtp_receive_workstep;	
	ntpMU._CMD_getStack_handler=igbtNtp_upload_stack;
	ntpMU._CMD_updateStack_handler=_NULL;
	ntpMU._CMD_controlChnl_handler=_NULL;
	ntpMU._CMD_sample_handler=igbtNtp_sample;
	ntpMU._CMD_stopAll_handler=_NULL;
	ntpMU._CMD_repair_handler=_NULL;
	ntpMU._CMD_setup_parallel_handler=_NULL;
	ntpMU._CMD_destroy_parallel_handler=_NULL;
	ntpMU._CMD_forceJump_handler=igbtNtp_force_jump;
	ntpMU._CMD_startChnl_handler=igbtNtp_start_test;
	ntpMU._CMD_stopChnl_handler=igbtNtp_stop_test;
	ntpMU._CMD_pause_handler=_NULL;
	ntpMU._CMD_continueTest_handler=_NULL;
	ntpMU._CMD_wcsInfo_handler=igbtNtp_receive_wcsi;
*/
	//ntp 事件消息
	message_map(CMD_Connect, &igbtNtpMU.ntpMsgResponders[0],igbtNtp_connect, &ntpMU.messageSet);
	message_map(CMD_WorkStepInfo, &igbtNtpMU.ntpMsgResponders[1],igbtNtp_receive_workstep, &ntpMU.messageSet);
	message_map(CMD_GetStack, &igbtNtpMU.ntpMsgResponders[2],igbtNtp_upload_stack, &ntpMU.messageSet);
	message_map(CMD_Sample, &igbtNtpMU.ntpMsgResponders[3],igbtNtp_sample, &ntpMU.messageSet);
	message_map(CMD_ForceJump, &igbtNtpMU.ntpMsgResponders[4],igbtNtp_force_jump, &ntpMU.messageSet);
	message_map(CMD_StartChnl, &igbtNtpMU.ntpMsgResponders[5],igbtNtp_start_test, &ntpMU.messageSet);
	message_map(CMD_StopChnl, &igbtNtpMU.ntpMsgResponders[6],igbtNtp_stop_test, &ntpMU.messageSet);
	message_map(CMD_WCSInfo, &igbtNtpMU.ntpMsgResponders[7],igbtNtp_receive_wcsi, &ntpMU.messageSet);
	message_map(CMD_ReqSubStepInfo, &igbtNtpMU.ntpMsgResponders[8],igbtNtp_reply_subStep_request, &ntpMU.messageSet);
	message_map(CMD_NetConnLost, &igbtNtpMU.ntpMsgResponders[9],igbtNtp_lost_connect, &ntpMU.messageSet);
	message_map(CMD_Pause, &igbtNtpMU.ntpMsgResponders[10],igbtNtp_pause_test, &ntpMU.messageSet);
	message_map(CMD_ContinueTest, &igbtNtpMU.ntpMsgResponders[11],igbtNtp_continue_test, &ntpMU.messageSet);
	message_map(CMD_SetupParallel, &igbtNtpMU.ntpMsgResponders[12],igbtNtp_setup_parallel, &ntpMU.messageSet);
	message_map(CMD_DestroyParallel, &igbtNtpMU.ntpMsgResponders[13],igbtNtp_destroy_parallel, &ntpMU.messageSet);
	message_map(CMD_ClearAllParall, &igbtNtpMU.ntpMsgResponders[14],igbtNtp_clear_all_parallel, &ntpMU.messageSet);
	message_map(ALARM_LIGHT_CMD, &igbtNtpMU.ntpMsgResponders[15],igbt_alarmled_cmd, &ntpMU.messageSet);
