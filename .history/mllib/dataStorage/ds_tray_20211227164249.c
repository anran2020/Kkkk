...
void ds_tray_file_init(mlu8 trayNum,mlu8 version,mlu32 baseDatSize,mlu32 datNum)
{
	...
	//一条数据占多少页计算
	mlu32 oneDatPerPage=((baseDatSize%DS_FLASH_PAGE_SIZE)>0)?(baseDatSize/DS_FLASH_PAGE_SIZE+1):(baseDatSize/DS_FLASH_PAGE_SIZE);
	mlu32 trayPageCnt=datNum*oneDatPerPage;//计算每个托盘，需要的flash页数
