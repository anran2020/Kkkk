...
//
//-----------------------------------------------------------------------------
MLBool ds_tray_write_chache(mlu8 traAddr,mlu8 datNum,mlu8* const pData)
{
	mlu8* pU8Dat=pData;
...
//
//-----------------------------------------------------------------------------
MLBool ds_tray_file_seek_end(mlu8 trayAddr,mlu32*curWriteLine)
{
	*curWriteLine=trayFile[trayAddr]->header.offsetLine;

	return trayFile[trayAddr]->header.coveredWriting;
}
...
mlu8 ds_tray_file_read_lines(mlu8 traAddr, mlu32 startline,mlu8 lineNum,mlu8*outDatBuf)
{
	...
	for (mlu8 l = 0; l < lineNum; l++)
	{
		//分页读取一行数据
		for(p=0;p<trayFile[traAddr]->pagesPerLine;++p)
		{
			pReadBuf=dsf_read_page(trayFile[traAddr],trayFile[traAddr]->basePage+DS_FILE_BACKUP_PAGE_NUM+pageAddr+i);			
			if(rPageBuf==nullptr)
			{
				return;
			}
