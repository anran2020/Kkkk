...
mlu8* ds_tray_disk_chache(mlu8 traAddr)
{
	...
	if(traAddr>=DS_TRAY_MAX||trayDiskCache[traAddr].isEmpty==0)
	{
...
void ds_tray_file_init(mlu8 trayNum,mlu8 version,mlu32 baseDatSize,mlu32 datNum)
{
	...
	for (i = 0; i < trayNum; ++i)
	{
		...
		pTrayCacheQue[i]=que_create(e_mem_sram,DS_TRAY_CACHE_NUM,sizeof(TrayDiskCache),trayDiskCache[i]);//创建数据缓存 
	}
