...
void ds_tray_file_init(mlu8 trayNum,mlu8 version,mlu32 baseDatSize,mlu32 datNum)
{
	...
	//文件一条数据，称为一行，文件按行读写
	//一行数据占多少页计算
	mlu32 linePerPage=((baseDatSize%DS_FLASH_PAGE_SIZE)>0)?(baseDatSize/DS_FLASH_PAGE_SIZE+1):(baseDatSize/DS_FLASH_PAGE_SIZE);
	mlu32 trayPageCnt=datNum*linePerPage;//计算每个托盘，需要的flash页数
	...
	for (i = 0; i < trayNum; ++i)
	{
		...
		trayFile[i]->linesPerPage=linePerPage;//一行数据有多少页
		trayFile[i]->linePageIndx=0;//行写入页索引
		trayFile[i]->lineMax=datNum;//文件大小，即文件有多少行

		dsf_open(trayFile[i],version);			//打开创建文件，根据版本号，打开 
