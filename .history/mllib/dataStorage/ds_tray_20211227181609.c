...
MLBool ds_tray_subtask(void)
{
	...
	for (i = 0; i < DS_TRAY_MAX; ++i)
	{
		...
		if (pCache==nullptr&&pCache->isEmpty)
		{
			continue; 									//队列无缓存
		}
		
		bufOffset=pCache->curWriteDiskPage;//计算缓存数据的偏移，即接着上次的数据写盘
		bufOffset=bufOffset*DS_FLASH_PAGE_SIZE;

		//一次写盘一页
		if(mlfalse==dsf_write_page(trayFile[i],pCache->pDat+bufOffset))
		{
	...
	return mlfalse;//返回false，标识没有写磁盘的 
