...
//
//-----------------------------------------------------------------------------
MLBool ds_tray_write_chache(mlu8 traAddr,mlu8 datNum,mlu8* const pData,mlu32 curWLine,mlu32 lastReadLine)
{
	mlu8* pU8Dat=pData;
	mlu32 i ;
...
mlu8 ds_tray_file_read_lines(mlu8 traAddr, mlu32 startline,mlu8 lineNum,mlu8*outDatBuf)
{
	...
	pageAddr=trayFile[traAddr]->basePage+DS_FILE_BACKUP_PAGE_NUM+pageAddr;

	log_print(LOG_LVL_INFO,"tra file r line=%d page=%d,num=%d ",startline,pageAddr,lineNum);
	for ( line = 0; line < lineNum; line++)
	{
		//分页读取一行数据
		for(p=0;p<trayFile[traAddr]->pagesPerLine;++p)
		{
			pReadBuf=dsf_read_page(trayFile[traAddr],pageAddr);			
			if(pReadBuf==nullptr)
			{
				return line;
			}
			...
			pageAddr++;
			if(pageAddr>=trayFile[traAddr]->endPage)//最后一页为开区间，属于下一个文件的起始页地址
			{
				pageAddr=trayFile[traAddr]->basePage+DS_FILE_BACKUP_PAGE_NUM;
			}
		}
	}
...
MLBool ds_tray_subtask(void)
{
	...
	for (i = 0; i < trayUsedNum; ++i)
	{
...
#if 1
		dsf_write_line(trayFile[i],pCache->line,pCache->pDat);	
#else		
		...
		if(pCache->curWriteDiskPage>=trayFile[i]->pagesPerLine)
		{
			//一条缓存数据写完
			pCache->curWriteDiskPage=0;
			pCache->isEmpty=1;
...
void ds_tray_file_init(mlu8 trayNum,mlu8 version,mlu32 baseDatSize,mlu32 datNum)
{
	...
	for (i = 0; i < trayNum; ++i)
	{
		...
		dsf_open(trayFile[i],version+3);			//打开创建文件，根据版本号，打开文件
		...
		for (j = 0; j < DS_TRAY_CACHE_NUM; j++)
		{
			...
			trayDiskCache[i][j].line=DS_FILE_LINE_NULL;//空行号，不执行写入
