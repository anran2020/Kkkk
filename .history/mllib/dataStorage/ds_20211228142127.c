...
//
//-----------------------------------------------------------------------------
MLBool ds_file_seek_end(DSFilileHandle fhdl,mlu32*curWriteLine,mlu32* lastReadLine)
{

	//托盘数据文件
	if(fhdl<e_dsf_trayMax)
	{
		return ds_tray_file_seek_end(fhdl,curWriteLine,lastReadLine);
	}
...
//
//-----------------------------------------------------------------------------
MLBool ds_write_file(DSFilileHandle fhdl,mlu8 datNum,mlu8*datBuf,mlu32 lastReadLine)
{
	mlu8 i;
	...
	if(fhdl<e_dsf_trayMax)
	{
		log_init(LOG_LVL_INFO,"ds_tray_write_chache,");
		return ds_tray_write_chache(fhdl,datNum,datBuf,lastReadLine);
	}
...
//
//-----------------------------------------------------------------------------
mlu8 ds_read_file(DSFilileHandle fhdl,mlu32 seq,mlu8 num,mlu8*datBuf)
{
	if(fhdl<e_dsf_trayMax)
	{
		return ds_tray_file_read_lines(fhdl, seq,num,datBuf);
	}
