...
//数据与数组之间的转换
//----------------------------------------------------------------------------
static inline void u8buf_to_dat(mulu8*u8buf,mlu8*dat,mlu8 bytes)
{
    mlu32 u32Dat;
    u32Dat=u8buf[3];
...
static inline void dat_to_u8buf(mlu32 u32Dat,mulu8*u8buf,mlu8 bytes)
{
    u8buf[0]=u32Dat;
    u8buf[1]=(u32Dat>>8);
    u8buf[2]=(u32Dat>>16);
...
static inline mlu16 u8buf_to_u16(mulu8*u8buf,)
{
    mlu16 u16Dat;
    u16Dat=u8buf[1];
    u16Dat=(u16Dat<<8)+u8buf[0];
    return u16Dat;
}

static inline void u16_to_u8buf(mlu16 u16Dat,mulu8*u8buf)
{
    u8buf[0]=u16Dat;
    u8buf[1]=(u16Dat>>8);
}
