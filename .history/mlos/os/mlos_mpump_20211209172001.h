...
typedef struct MsgConsumerItem{
	...
	struct MsgConsumerNode *pnext;						//指向下一个消费者
	STRUCT_INHERIT(MsgConsumerItem,SLListItemBaseStruct)//继承单向链表item属性，即可
...
typedef struct MessageListNode{
	...
	struct MessageListNode *pnext;					//指向下一个消息
	mlu8 idn;										//消息id number，有效值范围1-255,=0标识为空，0为非法idn
	mlu8 u8rsv;										//未使用字段
	mlu16 u16rsv;									//未使用字段
	ListPtr pConsumerList;							//消息消费链表，一个消息可以被多个对象消费

}MessageNode,MsgNode,*MsgNodeptr;
