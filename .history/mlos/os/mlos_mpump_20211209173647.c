...
//
//----------------------------------------------------------------------------
MsgNodeptr list_message_find(MsgPumpptr pMsgpump,MsgIDN idn)
{
	MsgNodeptr pMsgNode;
...
//
//----------------------------------------------------------------------------
MsgNodeptr list_message_register(MsgPumpptr pMsgpump,MsgIDN idn)
{
	MsgNodeptr pMsgNode=nullptr;


	pMsgNode=list_message_find(pMsgpump,idn);											//判断消息是否已经存在消息泵链表里
	if(pMsgNode==nullptr)																//消息不存在泵链表里,新建一个消息
	{
		pMsgNode=mlos_malloc(e_mem_sram,sizeof(MessageNode));
		pMsgNode->pnext=nullptr;
		pMsgNode->pConsumerList=list_create(e_mem_sram,e_lst_sortUnordered, e_lst_linkSingly);
		pMsgNode->idn=idn;
