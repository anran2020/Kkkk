...
//数据与数组之间的转换
//----------------------------------------------------------------------------
static inline void u8buf_to_dat(mulu8*u8buf,void*pdat,mlu8 bytes)
{
    mlu8* datptr=pdat;
    for (mlu8 i = 0; i < bytes; i++)
    {
       *datptr++= *u8buf++;/* code */
    }
...
static inline void dat_to_u8buf(void*pdat,mulu8*u8buf,mlu8 bytes)
{
    mlu8* datptr=pdat;
    for (mlu8 i = 0; i < bytes; i++)
    {
