...
//----------------------------------------------------------------------------
//链表模式的消息，以链表的形式管理消息，消息idn可不连续，可动态增加消息
//----------------------------------------------------------------------------
//#pragma anon_unions
typedef struct ListModeMsgItem{

	STRUCT_INHERIT(ListModeMsgItem,SLListItemBaseStruct)//继承单向链表item属性，
	mlu8 idn;										//消息id number，
	mlu8 u8rsv;										//未使用字段
	mlu16 u16rsv;									//未使用字段
	ListPtr pConsumerList;							//消息消费链表，一个消息可以被多个对象消费

}ListModeMessage,*ListModeMsgptr;

//----------------------------------------------------------------------------
//数组模式消息，以数组的形式管理消息，提前预知消息数量，不可动态增加，并消息idn连续
//----------------------------------------------------------------------------
//#pragma anon_unions
typedef struct {

	//数组管理消息，每个消息的idn是消息所在数组的索引
	
	ListPtr pConsumerList; 				//消息消费链表，一个消息可以被多个对象消费

}ArrayModeMessage,*ArrayModeMsgptr;
...
enum{
	...
	e_pump_msg_list=0,				//链表管理消息，消息id可以不连续
