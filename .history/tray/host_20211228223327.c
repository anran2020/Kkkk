...
void keepAliveChk(Timer *timer)
{
    if (0 == gUpItfCb->rxUpMsgCnt)
    {
        if (gUpItfCb->upOnline)
        {
            timerStart(&gUpItfCb->devUpDiscTmr, TidUpDisc, 30*60*1000, WiReset);
        }
...
void upMsgSmplTray(u8 *cmdPld, u16 cmdPldLen)
{
    ...
    log_print(LOG_LVL_INFO,"upper r tray[%d] seq=%d ",cmd->trayIdx,cmd->smplSeq);
    ack->smplAmt = 1;
    seq = findSmplBufSeq(cmd->smplSeq, smplMgr);
    seq=24;
    if (seq < smplMgr->smplBufAmt)
    {
        /*memcpy(ack->traySmplRcd, smplMgr->smplBufAddrBase+cmd->smplSeq*smplMgr->smplItemSize, smplMgr->smplItemSize);*/
        memcpy(ack->traySmplRcd, smplMgr->smplBufAddrBase+seq*smplMgr->smplItemSize, smplMgr->smplItemSize);
    }
    else
    {
    #ifdef DebugVersion
    ...
    if (!smplMgr->smplEnable)
    {
        ...
        seq = 0==cmd->smplSeq ? smplMgr->smplSeqMax : cmd->smplSeq-1;
        if (seq != smplMgr->smplSeqNext) /*只要不相等就解禁,不用管其它*/
        {
            smplMgr->smplEnable = True;
        }
    }
...
u16eRspCode upMsgFlowStartChk(UpFlowCtrlCmd *upCmd, u16 pldLen, DevMgr *dev)
{
    ...
    if (!tmprSmplBeOnline(tray->trayIdx) || !plcBeOnline(tray->plcIdx))
    {
        return RspDisc;
    }
...
void upMsgFlowStart(u8 *pld, u16 pldLen)
{
    ...
    step = (UpStepInfo *)getChnStepInfo(0);
    tray = &dev->tray[upCmd->trayIdx];
    if (0 == upCmd->chnAmt)
    {
        Channel *chnCri;
...
u16eRspCode upMsgFlowPauseChk(UpFlowCtrlCmd *upCmd, u16 pldLen, DevMgr *dev)
{
...
void upMsgFlowPause(u8 *pld, u16 pldLen)
{
    ...
    dev = gDevMgr;
    upCmd = (UpFlowCtrlCmd *)pld;
    if (RspOk != (rspCode=upMsgFlowPauseChk(upCmd, pldLen, dev)))
    {
        goto rspUp;
    }
    ...
    if (0 == upCmd->chnAmt)
    {
        ...
        for (chn=tray->chn,chnCri=&tray->chn[tray->trayChnAmt]; chn<chnCri; chn++)
        {
            if (ChnStaRun==chn->chnStateMed || ChnStaFlowStart==chn->chnStateMed
                || ChnStaLowSwStart==chn->chnStateMed || ChnStaLowStart==chn->chnStateMed)
            {
    ...
    else
    {
        ...
        for (chnIdx=0; chnIdx<upCmd->chnAmt; chnIdx++)
        {
            chn = &tray->chn[upCmd->chnId[chnIdx] / (tray->boxCfg.chnCellAmt+1)];
            if (ChnStaRun==chn->chnStateMed || ChnStaFlowStart==chn->chnStateMed
                || ChnStaLowSwStart==chn->chnStateMed || ChnStaLowStart==chn->chnStateMed)
            {
                chn->chnStateMed = ChnStaUpPauseReq;
...
u16eRspCode upMsgFlowStackChk(UpFlowCtrlCmd *upCmd, u16 pldLen, DevMgr *dev)
{
    ...
    {
        for (chnInd=upCmd->ctnuInd,sentry=&upCmd->ctnuInd[upCmd->chnAmt]; chnInd<sentry; chnInd++)
        {
            step = (UpStepInfo *)getChnStepInfo(chnInd->ctnuStepId);
            if (NULL==step || getStepTime(chnInd->ctnuStepId)<chnInd->stepRestTime)
            {
                return RspParam;
            }
            if (0 == chnInd->stepRestTime)
            {
                if (NULL == getChnStepInfo(chnInd->ctnuStepId+1))
                {
                    return RspParam;
                }
            }
...
void upMsgFlowStack(u8 *pld, u16 pldLen)
{
    ...
    else
    {
        for (chnInd=upCmd->ctnuInd,sentry=&upCmd->ctnuInd[upCmd->chnAmt]; chnInd<sentry; chnInd++)
        {
            ...
            if (0==chnInd->stepRestTime || getStepCap(chnInd->ctnuStepId)<=chn->capCtnu)
            {
            ...
            else
            {
                ...
                chn->stepRunTimeCtnu = getStepTime(chnInd->ctnuStepId) - chnInd->stepRestTime;
