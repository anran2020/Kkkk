...
void Pb_plc_socket_init(mlu8 plcx,mlu8 *remoteip, mlu16 remoteport,mlu16 (*datagramProcesses)(mlu8 *msg,mlu16 len))
{
    ...
    //创建socket，pb
    prsBed.usock = usocket_create (w5500_network_card(), e_sck_md_tcp, PLC_TX_BUF_SIZE, PLC_RX_BUF_SIZE);
    prsBed.usock->remoteport = remoteport;
    prsBed.usock->myport = 20207;
...
void Pb_init(Netcardptr pNetHardWare,mlu8 *remoteip, mlu16 remoteport,mlu16 (*rxcallback)(mlu8 *msg,mlu16 len))
{
    ...
    //创建任务
    prsBed.pMytask = task_create (PLC_TASK_PRIO, Pb_task, &prsBed, ltimer_create(),"upper");

    //消息泵创建
    prsBed.RxCallBack = rxcallback;

    //创建socket，pb
    prsBed.usock = usocket_create (pNetHardWare, e_sck_md_tcp, PLC_TX_BUF_SIZE, PLC_RX_BUF_SIZE);
    prsBed.usock->remoteport = remoteport;
    prsBed.usock->myport = 20207;
