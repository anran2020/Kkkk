...
Taskstate Pb_task(void *args)
{
    ...
    for(i=0;i<PB_CTRL_DEV_MAX)
    {
        if()
        {
            ...
            //提取消息数据包
            if(prsBed.datagramProcesses[i] != nullptr && prsBed.usock->rxDataLen != 0)
            {
                prsBed.usock->rxDataLen = prsBed.datagramProcesses[i](prsBed.usock[i]->pRxBuf,prsBed.usock->rxDataLen);
            }
