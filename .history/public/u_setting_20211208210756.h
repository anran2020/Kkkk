...
typedef struct
{
	...
	mlu8 plcNum;		//plc的数量
	mlu8 plc1MapTrayNum;			//plc映射的托盘数，即一个plc对应几个托盘
	mlu8 plc1MapTrayStartAddr;	//plc对应的托盘的起始地址

	mlu16 plc1ReadBaseAddr[4];//一个plc最大4个托盘
	mlu16 plc1ReadMaxAddr[4];

	mlu32 endFlag1;//结束标识，增加内容需要往后移动，到最后
	mlu32 endFlag2;

	mlu32 page2U32Rsv[2];//预留后续扩展使用，4字节对齐
